...
#Sumudu Tennakoon (Created: 2017-10-15, Last update: 2017-10-19)
...
class UnitCell:
    ...
    #Cartesian (xyz) to Fractional (uvw) Ref: https://en.wikipedia.org/wiki/Fractional_coordinates
    def M(self): 
        a = self.a
        b = self.b
        c = self.c
        ...
        return np.matrix([[a  , b * cos(gamma)    ,c * cos(beta)], \
                         [0   , b * sin(gamma)    ,c * (cos(alpha)-cos(beta)*cos(gamma))/sin(gamma)],\
                         [0   , 0                 ,self.volume()/(a*b*sin(gamma))]])
    #unit cell volume                     
    def volume(self):
        a = self.a
        b = self.b
